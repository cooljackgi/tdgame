...
export default function CoopGameLoader() {
        ...
        const joinAndListen = async () => {
            try {
                ...
                gameUnsub = onSnapshot(gameDocRef, (snap) => {
                    ...
                    const hasTwoPlayers = playersData.length === 2 && playersData[1].id === 'player2';
                    ...
                    if (role === 'player1' && !gameDataLoaded) {
                        ...
                        setGameStatus(hasTwoPlayers ? 'playing' : 'waiting'); // Start "playing" in intermission if P2 is there
                        ...
                        setPortals(data.portals || []);
                         ...
                         setGameStatus(hasTwoPlayers ? 'playing' : 'waiting');
                    }
  ...
  useEffect(() => {
      ...
      const gameLoop = () => {
          ...
          // Worker logic should run even in 'waiting' state if P2 is present.
